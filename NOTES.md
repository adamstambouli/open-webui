...
**Design artifacts** (repo root, self-contained, open from disk — kept deliberately as process evidence): `widget-showcase.html` renders the final design live, one annotated card per state; `widget-playground.html` is the iteration sandbox the design went through (11 rounds of side-by-side compares); `DESIGN.md` records the principles, systems, and the bugs the design process caught before they shipped.
